...
const ViewJobs = () => {
  ...
  const saveJob = (id, reminder) => {
    ...
    let savedJobs = jobs.map((job) =>
      job.id === id ? { ...job, remind_me: !job.remind_me } : job
    )
    setJobs(savedJobs)
  }
  ...
  const refreshIndeed = () => {
    ...
    fetch("http://localhost:3000/indeed")
      ...
      .catch((err) => {
        setLoading(!loading)
        alert(
          "you'll need to refresh the page and try again, indeed has a lot of popups that mess with the scrape method sometimes."
        )
      })
  }
  ...
  const filteredJobs = jobs.filter(
    (single_job) =>
      !single_job.job_title.toLowerCase().includes("senior") &&
      !single_job.job_title.toLowerCase().includes("sr") &&
      !single_job.job_title.toLowerCase().includes("lead") &&
      !single_job.job_title.toLowerCase().includes("manager") &&
      !single_job.job_title.toLowerCase().includes("director") &&
      !single_job.job_title.toLowerCase().includes("vp") &&
      !single_job.job_title.toLowerCase().includes("vice president") &&
      !single_job.job_title.toLowerCase().includes("principal") &&
      !single_job.job_title.toLowerCase().includes("architect")
  ...
  return (
    <>
      <header className="masthead">
        <div className="site-heading">
          ...
          <span className="subheading">
            Current listings for full stack developer and software engineering
            jobs in the past 24hrs. <br /> NO senior / sr / lead / VP / Director
            / Principal listings.
          </span>{" "}
          <br />
          <button className="btn-refresh" onClick={() => refreshLinked()}>
            Refresh Linkedin
          </button>
          <button className="btn-refresh" onClick={() => refreshNyc()}>
            Refresh Built in NYC
          </button>
          <button className="btn-refresh" onClick={() => refreshIndeed()}>
            Refresh Indeed
          </button>
      ...
      {loading ? (
        <BarLoader css={override} size={50} width={400} color={"#393a5f"} />
      ) : null}
